...
def run(parser,args):
	...
	if c.error != 'nanopore2023' and c.error != 'nanopore2020' and c.error != 'nanopore2018' and c.error != 'pacbio2016': #then, should be a file
		...
		if not os.path.exists(c.error):
			...
			now=datetime.now().strftime('%d/%m/%Y %H:%M:%S')
			print('[' + now + '][Error] Specified error model is not one of the accepted preset ("nanopore2023", "nanopore2020", "nanopore2018" or "pacbio2016") neither is a path to a trained error model for badread')
			sys.exit(1)

	c.quality=args.qscore_model

	if c.quality != 'nanopore2023' and c.quality != 'nanopore2020' and c.quality != 'nanopore2018' and c.quality != 'pacbio2016': #then, should be a file
		...
		if not os.path.exists(c.quality):
			...
			now=datetime.now().strftime('%d/%m/%Y %H:%M:%S')
			print('[' + now + '][Error] Specified quality model is not one of the accepted preset ("nanopore2023", "nanopore2020", "nanopore2018" or "pacbio2016") neither is a path to a trained quality model for badread')
			sys.exit(1)
